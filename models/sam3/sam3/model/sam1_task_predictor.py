...
class SAM3InteractiveImagePredictor(nn.Module):
    ...
    def predict_batch(
        ...
        for img_idx in range(num_images):
            ...
            masks_np = masks.squeeze(0).float().detach().float().cpu().numpy()
            iou_predictions_np = (
                iou_predictions.squeeze(0).float().detach().float().cpu().numpy()
            )
            low_res_masks_np = low_res_masks.squeeze(0).float().detach().float().cpu().numpy()
    ...
    def predict(
        ...
        masks_np = masks.squeeze(0).float().detach().float().cpu().numpy()
        iou_predictions_np = iou_predictions.squeeze(0).float().detach().float().cpu().numpy()
        low_res_masks_np = low_res_masks.squeeze(0).float().detach().float().cpu().numpy()
